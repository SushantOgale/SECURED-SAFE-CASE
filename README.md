# SecureVault: Encrypted File Transfer & Secure File Storage

SecureVault is a cybersecurity project that implements encrypted file upload/download between a client and server.
...
- MVP user isolation using `X-User` header
...
```bash
cd securevault_project
python client/securevault_client.py --user sushant upload sample.pdf
```
...
```bash
python client/securevault_client.py --user sushant list
```

## Download File

```bash
python client/securevault_client.py --user sushant download YOUR_FILE_ID --out downloads
```
...
```bash
python client/securevault_client.py --user sushant resume YOUR_FILE_ID sample.pdf
```
